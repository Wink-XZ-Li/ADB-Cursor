...
if ($Rebuild) {
    Get-ChildItem $Build -File -ErrorAction SilentlyContinue | Remove-Item -Force
}
...
Write-Host "+ BL51"
& "$C51Bin\BL51.exe" "build\STARTUP.obj,build\main.obj,build\log_uart.obj" "TO" "build\StageMinus1" "RAMSIZE(256)"
if ($LASTEXITCODE -gt 1) {
    throw "BL51 failed with exit $LASTEXITCODE"
}

Write-Host "+ OH51"
& "$C51Bin\OH51.exe" "build\StageMinus1" "HEXFILE(build\StageMinus1.hex)"
Assert-LastExit "OH51"

$hex = Join-Path $Build "StageMinus1.hex"
$h = Get-FileHash -LiteralPath $hex -Algorithm SHA256
Copy-Item -LiteralPath $hex -Destination (Join-Path $Rel "StageMinus1.hex") -Force
Write-Host "HEX $hex"
